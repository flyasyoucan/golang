...
func inCallToMobile(notifyMessage *[]byte) {
	...
	log.Debug("New called is:", inCall.Callee)
	if true != ipccClient.AcceptCall(inCall.CallId) {

		log.Error("answer the call failed!")
		return
	}

	ok := ipccClient.TransferToClient(inCall.CallId, "18676775495", "", "", "")

	if true != ok {
		//转接失败的情况处理
		log.Error("call client failed!", err)
...
func inCallProcByMobile(inCall ipcc.InCall) {
	...
	log.Debug("get callee voice:", info.GetCalleeVoice(), calleeVoice)
...
func dtmfProc(notifyMessage []byte) {
	...
	if nil != err {
		log.Error("get call failed:", callId)
		return
	}
	...
	if nil != err {
		//获取超时 或者有错误 直接挂机，并播放您拨打的号码是空号
		ipccClient.PlayVoice(callId, noNumberVoice)
...
func callStatProc(notifyMessage []byte) {
	...
	log.Debug("get call stat:", stat.Status)
...
func endCallProc(notifyMessage *[]byte) {
	...
	//更新结束时间
	log.Debug("update end time .recordfile:", record)
...
func chargeCalculate(bill ipcc.Bill) string {
...
func billMsgProc(notifyMessage *[]byte) {
	...
	if nil != err {
		log.Error("bill info not find!", err)
		return
	}

	if ok := mysql.Bill2Sql(&call); !ok {
		log.Error("save to db failed:%s", err.Error())
	}
...
func eventProcToMobile(w http.ResponseWriter, r *http.Request) {
	...
	case strings.EqualFold(eventCallBill, eventType):
		go callList.DelCall(event.CallId)
	}
}
