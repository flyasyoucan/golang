...
func AcceptCall(callid string) bool {
	...
	if err != nil {
		log.Error("encode answer call body failed!", err)
		return false
	}
	...
	log.Debug("answer the call", callid)
...
func TransferToClient(callId string, called string, calleeVoice string, callerVoice string, callerNum string) bool {
	...
	fmt.Println("transfer request:", string(toCallRequest))
	...
	log.Debug("transfer to client", callId, "called:", called, "caller",
		callerNum, "calleeVoice", calleeVoice, "callerVoice:", callerVoice)

	log.Debug("transfer result:", string(resp))
...
func GetDtmf(callid string, playFile string) bool {
	...
	log.Debug("playfile:", playFile)

	dtmfReq, err := ipcc.GetDtmfByTtsInterface(callid, playFile, "4", "#", "incall")
	if err != nil {
		log.Error("encode get dtmf body failed!", err)
		return false
	}
	...
	log.Debug("get dtmf", callid, playFile)
...
func HungUpTheCall(callid string) bool {
	...
	if err != nil {
		log.Error("encode request body failed!", err)
		return false
	}
...
func PlayVoice(callid string, fileName string) bool {
	...
	log.Debug("play voice", callid, fileName, "result:", string(resp))
